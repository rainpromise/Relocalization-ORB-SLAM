...
{
public:
  BriefData(MapPoint* pMapPoint, Frame* F=NULL, Vector<2> p2=Vector<2>())
    : BriefDesc(), pt(*pMapPoint), mpKF(F), v2p(p2)
  {
    pt.pBData=this;
  };
  ...
  Frame* mpKF; // in which KeyFrame was the BriefData extracted
  ...
  {
    uint32_t seq=0;
    if(mpKF!=NULL) seq=mpKF->mnId;
    std::stringstream out;
    out<<pt.GetWorldPos()[0]<<"\t"<<pt.GetWorldPos()[1]<<"\t"<<pt.GetWorldPos()[2]<<"\t";
    out<<seq<<"\t"<<v2p[0]<<"\t"<<v2p[1]<<"\t";
  ...
  virtual bool readFromMatlab(std::ifstream& in)
  {
    ...
    if(!(in>>pt.GetWorldPos()[0]))
      return false;
    else{
      in>>pt.GetWorldPos()[1]>>pt.GetWorldPos()[2];
      in>>dummySeq>>v2p[0]>>v2p[1];
      for (size_t i=0; i<BRIEF_K; ++i)
        in>>bd[i];
      if(mpKF!=NULL) mpKF->mnId=dummySeq;
      return true;
    }
  };
*/
...
public:
  ...
  BriefDataM(MapPoint* mp, Frame* kF, uint8_t* bd_)
  : BriefData(mp,kF)
  {
    bd=bd_;
  };
  BriefDataM(MapPoint* mp, Frame* kF, Vector<2> p2, uint8_t* bd_)
  : BriefData(mp,kF,p2)
  {
    bd=bd_;
  };
