...
//      mp->pBData=brief;
      mp->mWorldPos[0]=w_x;
      in>>mp->mWorldPos[1]>>mp->mWorldPos[2]>>frameNr>>brief->v2p[0]>>brief->v2p[1];
    ...
    map<BriefData*,size_t>& briefFrames)
{
  ...
  {// load brief data from files
    ...
    while(in>>id)
    {
      ...
      BriefDataS* brief=new BriefDataS(mp);
      int32_t frameNr=-1;
      in>>mp->mWorldPos[0]>>mp->mWorldPos[1]>>mp->mWorldPos[2]>>frameNr>>brief->v2p[0]>>brief->v2p[1];
      for (size_t j=0; j<BriefData::BRIEF_K/4; ++j){
        uint32_t d;
        in>>d;
        brief->bd[j*4]=uint8_t(d>>24);
  ...
  {// load brief data from files
    ...
    while(in>>id)
    {
      ...
      { // we already opened a new bucket for the BRIEF descriptors of this MapPoint
        ...
        mp=new MapPoint();
        mp->mWorldPos[0]=x; // only need to assign them once since the d pos remains the same and equal to the last estimate of the 3D pos
        mp->mWorldPos[1]=y;
        mp->mWorldPos[2]=z;
      }
